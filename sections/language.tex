...
\medskip
\noindent\textbf{Note.} Unless otherwise indicated, all Examples in this chapter and the chapters that follow (Chapters~3–6) are drawn from the Middle Polish Dependency Treebank (MPDT).
...
\begin{itemize}[leftmargin=2em]
  \item Historical joint writing \textit{zchęći}, modern \textit{z chęci} (`from willingness')
  \item Historical separate writing \textit{dla tego}, modern \textit{dlatego} (`because', lit. `for this')
\end{itemize}

Following \textcite{Wieczorek2020Instr}, the MPDT treats historical spacing as evidence but bases syntactic analysis on function. Two frequent scenarios occur. (i) Historically fused sequences that correspond to ordinary prepositional phrases are split into their syntactic parts; the preposition and its nominal complement are annotated as in regular usage (see \exref{ex:orth-pp}). (ii) Historically separate sequences that function as a single connective or adverbial are kept as two tokens but marked as a fixed multiword unit in MPDT; the unit then receives a single clausal function (see \exref{ex:orth-fixed}).

\begin{example}\label{ex:orth-pp}
\textit{Poniewoli musiała zbita na łożu leżeć u rodziców/ ale \textbf{zchęci} obiecała sobie inną okazją/ iż jej od przedsięwzięcia nie oderwą.}\\
...
In \exref{ex:orth-pp}, \emph{zchęci} is analyzed in MPDT as the preposition \emph{z} with the genitive noun \emph{chęci}: the preposition functions as a modifier of the non-finite predicate, and the noun is its complement.

\begin{example}\label{ex:orth-fixed}
\textit{Tak jest: i \textbf{dla tego} tak się poniżył.}\\
(`Indeed: and therefore he humbled himself thus.')
\end{example}

In \exref{ex:orth-fixed}, \emph{dla tego} forms a lexicalized connective in MPDT: \emph{dla} serves as the syntactic head of the unit that modifies the clause, and \emph{tego} is linked to it as a multiword element within that unit.
...
In \exref{ex:dual-dwie-lecie}, \textit{dwie lecie} is dual accusative of \textit{dwa} (two) and \textit{lato} (`summer; year'). In modern Polish, the dual form is archaic; the standard form (both the nominative and accusative) is \textit{dwa lata}.
...
As noted by \textcite{Wieczorek2025TowardsMPDT}, it is rare in modern Polish for sentences to lack a predicate (at least in texts written in careful language), but this was quite common in 17th–18th-century Polish. In dependency analysis, the predicate is considered the centre of the sentence (\texttt{root})—most often a finite verb. In the absence of a predicate, another sentence element serves as the centre. Most often, this centre becomes the subject, which receives the label \texttt{root} instead of \texttt{subj}, as in Figure \ref{fig:ellipsis}.

\begin{figure}[H]
  \centering
  \resizebox{\textwidth}{!}{%
    {\scriptsize
    \begin{dependency}[theme=simple, baseline=7.2em, label style={font=\footnotesize}]
      \begin{deptext}[column sep=2.2em, nodes={text width=7.0em, align=center}]
        Tabakierka \& złotem \& wybijana \& . \\
        \textit{snuffbox} \& \textit{with gold} \& \textit{embossed} \& \textit{.} \\
         \& \& \& \\
        SUBST \& SUBST \& PPAS \& INTERP \\
      \end{deptext}
  ...
  \caption{Predicate ellipsis: nominal head as \texttt{root}\\
  \textit{Source: adapted from \textcite{Wieczorek2025TowardsMPDT}, Fig.~8, p.~13.}}
  \label{fig:ellipsis}
\end{figure}
...
\textit{jako}. It is frequently employed in two functions: (i) as a comparative/similative marker in the sense of modern \textit{jak} (`like/as; when/how'), and (ii) in the modern-like role/identity sense `as'. The readings are illustrated in Examples \ref{ex:jako-when}–\ref{ex:jako-role}.

\begin{example}\label{ex:jako-when}
...
\begin{example}\label{ex:jako-as}
...
\begin{example}\label{ex:jako-role}
...
\end{example}
In \exref{ex:jako-when}, \textit{jako} functions as a clause linker with temporal meaning (`when'). In \exref{ex:jako-as} it introduces a comparative clause (`how/as'), and in \exref{ex:jako-role} it introduces a role/identity complement (`as (in the role of)'). In modern Polish, the first two functions are typically expressed with \textit{jak}, while the third remains \textit{jako}.
