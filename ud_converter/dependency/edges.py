            ...
            if sgov and int(sgov.id) != 0:
                ...
                for dep in t.children:
                    if dep.lemma != 'nie' and dep.dep_label != 'neg' and dep.udep_label != 'conj':
                        dep.ugov = sgov
                        logger.debug("Reattached dependent %s from auxiliary %s to %s", dep.form, t.form, sgov.form)
                ...
                for fixed_token in fixed_tokens:
                    for dep in fixed_token.children:
                        if dep.ugov != fixed_head:
                            dep.ugov = fixed_head
                            logger.debug("Reattached dependent %s from fixed token %s to %s", dep.form, fixed_token.form, fixed_head.form)
            else:
                logger.warning("Fixed token %s has no super-governor.", t.form)
                continue
        ...
        for flat in flat_deps:
            ...
            if int(flat.id) < int(t.id):
                if t.ugov:
                    flat.ugov = t.ugov
                    t.ugov = flat
                    logger.debug("Reordered flat structure: %s -> %s", flat.form, t.form)
                else:
                    logger.warning("Flat token %s has no UD governor.", t.form)
        ...
        if t.ugov and t.udep_label in ['mark', 'case', 'cc', 'cc:preconj']:
            ...
            for c in list(set(t.children + t.uchildren)):
                ...
                if (c.udep_label in ['punct', 'advmod', 'list', 'cop', 'obl', 'aux:clitic', 'advcl', 'mark', 'orphan'] and
                    ...
                    c.ugov = t.ugov
                    logger.debug("Reattached dependent %s from %s %s to %s", c.form, t.udep_label, t.form, t.ugov.form)
