...
def convert_coordination(s: Sentence) -> None:
    ...
    for t in s.post_order_tokens():
        gov = t.gov2
        if not gov:
            ChangeCollector.record(t.sentence.id, t.id, f"Skipping: Token '{t.form}' has no governor", module="structures.coordination0", level='WARNING')
            continue
        ...
        # Coordination with a punctuation mark used as a conjunction, e.g. "Siedzi, czyta."
            ...
            if t.udep_label != 'conj' and conjuncts and conjuncts[0].udep_label not in ['punct', 'cc']:
                ...
                if t.udep_label == 'conj' and t.children_with_label('conjunct'):
                    coordination(t, gov, True, ud_label='conj')
...
def coordination(t: Token, gov: Token, punct_conj: bool = False, ud_label: str | None = None) -> None:
    """
    ...
    :param Token gov: The governor token
    ...
    if punct_conj:
        ...
        if t.gov_id:
            ...
            if t.udep_label not in ['conj', 'punct']:
                main_c.udep_label = ud_label if ud_label else t.udep_label
            else:
                main_c.udep_label = '_'
            if t.dep_label not in ['conjunct', 'punct']:
    ...
    # Conversion of coordination structures with conjunctions
        ...
        # Attach the first conjunct to the governor
        ChangeCollector.record(t.sentence.id, main_c.id, f"Attaching first conjunct '{main_c.form}' to governor '{gov.form}' with dep_label '{t.dep_label}' and udep_label '{ud_label if ud_label else t.udep_label}'", module="structures.coordination7")
        main_c.ugov = gov
        main_c.gov = gov
        main_c.udep_label = ud_label if ud_label else t.udep_label
        main_c.dep_label = t.dep_label
        ...
        if next_conjunct:
            ...
            # Handle enhanced dependencies
            if gov and t.dep_label != 'conjunct' and cl(main_c, gov=gov) != '_' and gov.id not in main_c.eud:
                ChangeCollector.record(t.sentence.id, main_c.id, f"Adding eud {gov.id}: {cl(main_c)} to '{main_c.form}'", module="structures.coordination-eud3")
                main_c.eud = {gov.id: cl(main_c)}

    # Process other elements of the coordination structure
    process_puncts(puncts, conjuncts)
    process_conjuncts(conjuncts, main_c, t, gov)
...
def process_conjuncts(conjuncts: list[Token], main_c: Token, t: Token, gov: Token) -> None:
    """
    ...
    for c in conjuncts:
        if c != main_c:
            ...
            if t.dep_label == 'conjunct' and (gov != t.gov2 or t.udep_label == '_'):
                ...
                if gov.id not in c.eud and gov.upos == 'CCONJ':
                    enhanced_conjuncts = []
                    for govc in gov.children_with_label('conjunct'):
...
def process_precoords(pre_coords: list[Token], conjuncts: list[Token]) -> None:
    ...
    for pre in pre_coords:
        ...
        if later_conjuncts:
            ...
            pre.gov_id = min([c.id for c in later_conjuncts])
            pre.udep_label = 'cc:preconj'
            pre.dep_label = 'pre_coord'
