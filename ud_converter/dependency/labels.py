...
def convert_label(t: Token) -> str:
    ...
    # subordinating conjunction
        ...
        logger.warning("Sentence %d: Multiple 'tyle'-lemma children for o: '%s'", t.sentence.id, t.form)
...
def verb_complement(t: Token, cleft: bool = False) -> str:
    ...
    if cleft:
        if t.upos == 'VERB':
            ...
            if cop:
                if len(cop) > 1:
                    logger.warning("Sentence %d: Multiple copula children for '%s': %s", t.sentence.id, t.form, cop)
                cop = cop[0]
                if cop.pos == 'inf':
                    return 'xcomp:cleft'
                return 'ccomp:cleft'
    ...
    if t.gov and t.gov.upos in ['PROPN', 'NOUN', 'PRON', 'X', 'NUM', 'SYM']:
        ...
        if t.upos == 'ADJ':
            if t.pos in ['ppas', 'pact']:
                if mark:
                    if len(mark) > 1:
                        logger.warning("Sentence %d: Multiple 'mark'-children for '%s': %s", t.sentence.id, t.form, mark)
                    mark = mark[0]
