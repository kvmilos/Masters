...
Middle Polish frequently employs conjunctions that have since changed meaning:
\textit{jako} ('as’, 'that’), \textit{iż} ('that’), \textit{abo} ('or’), \textit{a} (broad connective).  
As \textcite{Wieczorek2020Instr} notes, \textit{jako} may function either as a comparative marker or a subordinating conjunction equivalent to \textit{że}.  
During conversion, such cases are normalized to UD \texttt{mark} relations headed by \texttt{SCONJ}.  
\textbf{"TO CHECK THE JAKO”}.
