...
Middle Polish Dependency Treebank is a~syntactically annotated subset of~KorBa that adds a~dependency layer on~top of~KorBa's tokenization, lemmas,
POS, and FEATS for~selected Middle Polish texts \parencite{Wieczorek2025TowardsMPDT}.
The annotation scheme follows PDB conventions, making it compatible with contemporary Polish syntactic resources while addressing specific Middle Polish phenomena.

The resource is under active development and not yet publicly released. The current version contains 2,018 sentences, with an average sentence length of X tokens.

[Tutaj proszę scharakteryzować MPDT (typy tekstów, liczba drzew, średnia tokenów/zdanie, statystyki tagów, etykiet, krzyżujących się krawędzi, etc.). Podać przykłady.]
