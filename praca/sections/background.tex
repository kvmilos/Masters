...
\begin{enumerate}[leftmargin=2em]
  \item \textbf{Tokenization.} UD defines dependencies between \emph{syntactic words}.
        ...
        A multiword token is a single orthographic unit that is split into multiple syntactic words, each receiving its own morphological analysis and syntactic role.
...
15      m          być     AUX   ...
\end{verbatim}
        Here, the single orthographic token \textit{kiedym} (ID 14-15) splits into two syntactic words: \textit{kiedy} 'when' (ID 14) and \textit{m} (clitic form of 'I am', ID 15).
...
\end{verbatim}
  ...
  \item \textbf{Syntax.} The syntactic layer is a~single-rooted tree with possible
        ...
        Sentence structures are modeled as~directed trees whose nodes correspond to~syntactic words and whose
        edges encode governor–dependent links. Relations include:
        \begin{itemize}
        ...
        \item non-core dependents (oblique \texttt{obl}, dislocated element \texttt{dislocated}, adverbial clause modifier \texttt{advcl}, adverbial modifier \texttt{advmod}, discourse element \texttt{discourse}, auxiliary \texttt{aux}, and copula \texttt{cop}), 
...
To accommodate Middle Polish morphology, which is described in detail in \autoref{chap:language}, several POS and feature categories were added:
\begin{itemize}[leftmargin=2em]
